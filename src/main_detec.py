...
def main():
    ...
    #SCALER_PR = cdict['stats_conf']['PREPROCESS']['SCALER_PR']
    ...
    var: 
        (
        SCALER_PR if 'pr' in var else 
        (SCALER_HUS if 'hus' in var else SCALER_DEF)
        )
        for var in varname_predictor_low_res + varname_predictand_high_res
    }
    """
    ...
    print('X_test, const_test, y_test min:', np.min(X_test), np.min(const_test), np.min(y_test))
    print('X_test, const_test, y_test max:', np.max(X_test), np.max(const_test), np.max(y_test))
